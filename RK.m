function [x, y] = RK(a,b,h,con,gotler,baseT,baseTdash,baseU,kappa,...
    ...
    for k = 1:(n-1) 
        ...
        k2 = gotler(x(k)+0.5*h,y(:,k)+0.5*h*k1',baseT(k),...
            baseTdash(k),baseU(k),kappa,betag,Gstar,Q,sigma,intbaseT);
        k3 = gotler(x(k)+0.5*h,y(:,k)+0.5*h*k2',baseT(k),...
            baseTdash(k),baseU(k),kappa,betag,Gstar,Q,sigma,intbaseT);
        k4 = gotler(x(k)+h,y(:,k)+h*k3',baseT(k),baseTdash(k),...
            baseU(k),kappa,betag,Gstar,Q,sigma,intbaseT);
